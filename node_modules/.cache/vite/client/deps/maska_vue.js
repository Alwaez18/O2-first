...
var S = (n, s, t) => s in n ? I(n, s, { enumerable: true, configurable: true, writable: true, value: t }) : n[s] = t;
...
var P = (n, s = true, t) => {
  var A, y, g, d;
  const e = ((A = t.number) == null ? void 0 : A.unsigned) !== true && n.startsWith("-") ? "-" : "", a = ((y = t.number) == null ? void 0 : y.fraction) ?? 0;
  let r = C(0, a, t);
  ...
  let l2 = r.format(parseFloat(i));
  return s ? a > 0 && i.endsWith(".") && !i.slice(0, -1).includes(".") && (l2 += f) : l2 = R(l2, u, f), e + l2;
};
var F = class {
  constructor(s = {}) {
    b(this, "opts", {});
  ...
  unmasked(s) {
    return this.process(String(s), this.findMask(String(s)), false);
  }
  isEager() {
  ...
  process(s, t, e = true) {
    ...
    const { mask: r, escaped: h } = this.escapeMask(t), u = [], f = this.opts.tokens != null ? this.opts.tokens : {}, i = this.isReversed() ? -1 : 1, p = this.isReversed() ? "unshift" : "push", l2 = this.isReversed() ? 0 : r.length - 1, A = this.isReversed() ? () => o > -1 && c2 > -1 : () => o < r.length && c2 < s.length, y = (k2) => !this.isReversed() && k2 <= l2 || this.isReversed() && k2 >= l2;
    let g, d = -1, o = this.isReversed() ? r.length - 1 : 0, c2 = this.isReversed() ? s.length - 1 : 0, E = false;
    for (; A(); ) {
      const k2 = r.charAt(o), m = f[k2], M = (m == null ? void 0 : m.transform) != null ? m.transform(s.charAt(c2)) : s.charAt(c2);
      if (!h.includes(o) && m != null ? (M.match(m.pattern) != null ? (u[p](M), m.repeated ? (d === -1 ? d = o : o === l2 && o !== d && (o = d - i), l2 === d && (o -= i)) : m.multiple && (E = true, o -= i), o += i) : m.multiple ? E && (o += i, c2 -= i, E = false) : M === g ? g = void 0 : m.optional && (o += i, c2 -= i), c2 += i) : (e && !this.isEager() && u[p](k2), M === k2 && !this.isEager() ? c2 += i : g = k2, this.isEager() || (o += i)), this.isEager())
        for (; y(o) && (f[r.charAt(o)] == null || h.includes(o)); ) {
          if (e) {
            if (u[p](r.charAt(o)), s.charAt(c2) === r.charAt(o)) {
              o += i, c2 += i;
              continue;
            }
          } else r.charAt(o) === s.charAt(c2) && (c2 += i);
          o += i;
        }
    }
...
var x = (n) => n.startsWith("[") && n.endsWith("]") ? w(n) : n;
var W = (n) => {
...
var V = class {
  constructor(s, t = {}) {
    ...
    b(this, "onInput", (s2) => {
      if (s2 instanceof CustomEvent && s2.type === "input" && !s2.isTrusted && !s2.bubbles)
        return;
      const t2 = s2.target, e = this.items.get(t2), a = "inputType" in s2 && s2.inputType.startsWith("delete"), r = e.isEager(), h = a && r && e.unmasked(t2.value) === "" ? "" : t2.value;
  ...
  init(s) {
    ...
    for (const e of s) {
      if (!this.items.has(e)) {
        ...
        e.addEventListener("input", this.onInput, { capture: true, signal: r });
      }
      const a = new F(T(e, t));
  ...
  fixCursor(s, t, e) {
    ...
    let l2 = a;
    u !== f && (l2 += t ? h.length - r.length : i.length - p.length), s.setSelectionRange(l2, l2);
  }
  setValue(s, t) {
